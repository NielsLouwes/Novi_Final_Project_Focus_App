    ...
    return (
        <div className="register-background">
            <form onSubmit={handleSubmit} className="form-main" >
                ...
                {error && <p className="form-error" style={{ color: 'white' , margin: " 0 5px 0 15px" }}>{error}</p>}

                <button className="register__button" disabled={loading} type="submit">Log In</button>
                <p className="register__text">Don't have an account? <Link style={{ color: 'white' }} to="/register">Register</Link></p>
            </form>
        </div>
    );
}