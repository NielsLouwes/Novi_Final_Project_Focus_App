...
export default function  BinauralBeatsMotivation() {
    ...
    useEffect(() => {
        async function fetchBinauralBeatsMotivation () {
            try {
                const response = await axios.get(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&videoEmbeddable:true&maxResults=6&order=relevance&q=binauralbeats%20motivation&key=${process.env.REACT_APP_YT_API_KEY}`)
