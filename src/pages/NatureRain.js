...
export default function NatureSoundsRain() {
    ...
    useEffect(() => {
        async function fetchNatureSoundsRain() {
            try {
                const response = await axios.get(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&videoEmbeddable:true&maxResults=6&order=relevance&q=rainsounds&key=${process.env.REACT_APP_YT_API_KEY}`)
